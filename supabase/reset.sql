-- Project Gobi: Full Reset
-- Run this in the Supabase SQL editor to wipe everything and start clean.
-- Safe to run on a new project with no real user data.
...
DROP TABLE IF EXISTS vendor_shortlist_items CASCADE;
...
DROP TABLE IF EXISTS registry_links CASCADE;
DROP TABLE IF EXISTS photo_albums CASCADE;
DROP TABLE IF EXISTS guest_event_rsvps CASCADE;
...
-- ── Drop all enums ────────────────────────────────────
DROP TYPE IF EXISTS tradition CASCADE;
DROP TYPE IF EXISTS ceremony_type CASCADE;
DROP TYPE IF EXISTS rsvp_status CASCADE;
DROP TYPE IF EXISTS vendor_category CASCADE;
DROP TYPE IF EXISTS booking_status CASCADE;
DROP TYPE IF EXISTS payment_status CASCADE;
DROP TYPE IF EXISTS checklist_phase CASCADE;
DROP TYPE IF EXISTS vendor_urgency CASCADE;
DROP TYPE IF EXISTS wedding_size CASCADE;
DROP TYPE IF EXISTS subscription_tier CASCADE;
DROP TYPE IF EXISTS user_type CASCADE;
DROP TYPE IF EXISTS family_side CASCADE;

-- ── Delete any test users from auth ───────────────────
-- (Uncomment if you want to clear auth users too)
...
BEGIN
  ...
  VALUES (
    ...
    COALESCE(NULLIF(NEW.raw_user_meta_data->>'user_type', '')::user_type, 'couple')
  )
